...
test_that("Function issues warnings correctly", {
  ...
  expect_warning(
    ...
    regexp = "Requested 'motif_size' \\(3\\) is larger than the shortest available sequence \\(2\\). Size has been adjusted to 2."
  )
...
test_that("Custom axis labels are generated correctly for each motif_type", {
  ...
  expect_equal(label_data_start$label, as.character(1:2))
  ...
  expect_equal(label_data_end$label, as.character((-2):-1))
