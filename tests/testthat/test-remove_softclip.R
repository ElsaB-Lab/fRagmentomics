...
test_that("remove_softclip trims both ends and yields the expected fragment size", {
  ...
  rs5_orig <- list(FLAG = df_sam$FLAG[1], MAPQ = df_sam$MAPQ[1], TLEN = df_sam$TLEN[1],
                   CIGAR = df_sam$CIGAR[1], POS = df_sam$POS[1],
                   SEQ = df_sam$SEQ[1], QUAL = df_sam$QUAL[1], read_length = nchar(df_sam$SEQ[1]))
  rs3_orig <- list(FLAG = df_sam$FLAG[2], MAPQ = df_sam$MAPQ[2], TLEN = df_sam$TLEN[2],
  ...
  # After trimming:
  #  - 5' read was '5S20M'  -> '20M'
  #  - 3' read was '20M5S'  -> '20M'
  expect_identical(res_trim$CIGAR_5p, "20M")
  expect_identical(res_trim$CIGAR_3p, "20M")
  ...
  expect_identical(as.integer(sc_counts_after$nb_softclip_5p), 0L)
  expect_identical(as.integer(sc_counts_after$nb_softclip_3p), 0L)
