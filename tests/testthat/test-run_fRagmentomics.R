...
test_that("run_fRagmentomics works on standard inputs and returns a data frame", {
...
test_that("run_fRagmentomics handles different parameter settings correctly", {
  files <- setup_test_files()

  # Test with additional reporting columns enabled
  results_extra_cols <- run_fRagmentomics(
    mut = files$mut,
    bam = files$bam,
    fasta = files$fasta,
    report_tlen = TRUE,
  ...
  results_string_mut <- run_fRagmentomics(
    mut = "chr1:685:C:T",
    bam = files$bam,
    fasta = files$fasta,
    n_cores = 1
...
test_that("run_fRagmentomics handles file output logic correctly", {
  files <- setup_test_files()
  temp_out <- tempdir()
  ...
  results_no_output <- run_fRagmentomics(
    mut = files$mut, bam = files$bam, fasta = files$fasta,
...
test_that("run_fRagmentomics handles edge cases gracefully", {
  ...
  expect_warning(
    results_mixed <- run_fRagmentomics(
      mut = mut_mixed_file,
      bam = files$bam,
      fasta = files$fasta,
      n_cores = 1
