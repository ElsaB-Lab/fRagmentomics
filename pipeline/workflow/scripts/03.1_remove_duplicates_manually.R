...
RemoveDuplicates <- function(inputfile, sample, alternatebase, outputfile) {
  ...
  df_file <- lapply(inputfile, function(inputfile) {
    ...
    df$last_4_characters_inverted <- paste0(
      substr(x = df$last_4_characters, start = 3, stop = 4),
    ...
    df$UMI_characters <- paste0(
      pmin(df$last_4_characters, df$last_4_characters_inverted), "_",
