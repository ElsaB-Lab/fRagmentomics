...
results_df_basic <- run_fRagmentomics(
  mut = mut_file,
  bam = bam_file,
  fasta = fasta_file,
...
results_df_advanced <- run_fRagmentomics(
  mut = mut_file,
  bam = bam_file,
  fasta = fasta_file,
  sample_id = "cfdna-test-01",
...
| **`WT`** | The fragment confidently supports an allele that is **not the target**.      | `WT`/`WT`, `WT`/`AMB`, `WT`/`NA`, `OTH`/`OTH`, `OTH`/`AMB`, `OTH`/`NA`, or `WT`/`OTH`                          |
...
The `results_df_advanced` object, produced by `run_fRagmentomics()`, can be directly used with the package's suite of plotting functions to visualize the results.
...
```{r plot_size_histogram, fig.cap="Histogram comparing the fragment size distribution of MUT and WT fragments."}
...
```{r plot_motif_hierarchical, fig.cap="Hierarchical view of 3-base motif proportions for MUT and WT groups."}
plot_motif_barplot(
...
```{r plot_motif_differential, fig.cap="Differential analysis showing the log2 Fold Change of motif usage between MUT and WT groups."}
plot_motif_barplot(
  df_fragments = results_df_advanced,
...
```{r plot_logo_comparison, fig.cap="Sequence logos for the 5'-end 6-base motif, comparing MUT and WT fragments."}
plot_qqseqlogo_meme(
  df_fragments = results_df_basic,
  motif_type = "Start",
  motif_size = 6,
...
```{r plot_freq_barplot, fig.cap="Overall nucleotide frequency at the 5' end of MUT vs. WT fragments."}
plot_freq_barplot(
  df_fragments = results_df_basic,
  motif_size = 5,
  motif_type = "Start",
