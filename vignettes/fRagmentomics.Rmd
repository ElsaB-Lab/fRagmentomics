...
This vignette provides a step-by-step walkthrough of the main analysis workflow using the `analyze_fragments()` function and demonstrates the package's core visualization capabilities thank to the visualisation functions: `plot_size_distribution()`, `plot_qqseqlogo_meme()`, `plot_freq_barplot()`, `plot_motif_barplot()`.
...
To make it user-friendly, the entire analysis is performed by a single function: `analyze_fragments()`. We will use the example data included with the package to demonstrate a typical workflow.
...
First, we will run `analyze_fragments()` with the default arguments. We provide the paths to our three input files, a `sample_id` for annotation, and specify the number of cores for parallel processing.

```{r run_analysis_basic, message=TRUE, warning=TRUE}
# Run the full analysis pipeline with default settings
results_df_basic <- analyze_fragments(
  mut = mut_file,
  bam = bam_file,
  fasta = fasta_file,
...
cat("Dimensions of basic results:", dim(results_df_basic), "\n")
head(results_df_basic)
```

### Advanced Workflow: Customizing the Analysis

The `analyze_fragments()` function can be customized for more specific use cases. In the example below, we will perform a more stringent analysis and request additional information in the output table.
...
results_df_advanced <- analyze_fragments(
  mut = mut_file,
  bam = bam_file,
  fasta = fasta_file,
  sample_id = "cfdna-test-01",
...
cat("Dimensions of advanced results:", dim(results_df_advanced), "\n")
...
The `analyze_fragments()` function returns a `data.frame` where each row represents the analysis of a single fragment. Let's explore the key columns to understand the results.
...
| **`NON-TARGET MUT`** | The fragment confidently supports an allele that is **not the target**.      | `WT`/`WT`, `WT`/`AMB`, `WT`/`NA`, `OTH`/`OTH`, `OTH`/`AMB`, `OTH`/`NA`, or `WT`/`OTH`                          |
| **`DISCORDANT`** | The two reads give **conflicting** high-confidence information.            | `MUT`/`WT` or `MUT`/`OTH`                                                     |
| **`AMB`** | The evidence is **ambiguous**; a confident call cannot be made.            | `AMB`/`AMB`, `AMB`/`NA`                       |
...
```{r summary_size}
summary(results_df_advanced$Fragment_Size)
```
...
The `results_df_advanced` object, produced by `analyze_fragments()`, can be directly used with the package's suite of plotting functions to visualize the results.

## Fragment Size Distribution

One of the most common analyses is to compare the size distribution of fragments that carry the mutation (`MUT`) against those that do not (`NON-TARGET MUT`). The `plot_size_distribution()` function is designed for this purpose.
...
```{r plot_size_histogram, fig.cap="Histogram comparing the fragment size distribution of MUT and NON-TARGET MUT fragments."}
plot_size_distribution(
  df_fragments = results_df_advanced,
...
```{r plot_size_density, fig.cap="Density plot comparing the fragment size distribution of all available status groups."}
plot_size_distribution(
  df_fragments = results_df_advanced,
...
```{r plot_size_ungrouped, fig.cap="Overall fragment size distribution for all fragments, shown as a combined histogram and density plot."}
plot_size_distribution(
  df_fragments = results_df_advanced,
  col_z = NULL,
  show_histogram = TRUE,
...
```{r plot_motif_hierarchical, fig.cap="Hierarchical view of 3-base motif proportions for MUT and NON-TARGET MUT groups."}
plot_motif_barplot(
  df_fragments = results_df_basic,
...
```{r plot_motif_hierarchical_custom, fig.cap="A customized hierarchical plot filtered for specific starting bases."}
plot_motif_barplot(
  df_fragments = results_df_basic,
  representation = "split_by_base",
  motif_start = c("C", "G"),
...
```{r plot_motif_differential, fig.cap="Differential analysis showing the log2 Fold Change of motif usage between MUT and NON-TARGET MUT groups."}
plot_motif_barplot(
  df_fragments = results_df_advanced,
...
```{r plot_motif_side_by_side, fig.cap="Side-by-side comparison of motif proportions."}
plot_motif_barplot(
  df_fragments = results_df_basic,
...
A common use case is to compare the end-motif patterns of different fragment groups. Here, we'll look at the first 6 bases (`motif_size = 6`) of the 5' end (`motif_type = "Start"`) for `MUT` versus `NON-TARGET MUT` fragments.

```{r plot_logo_comparison, fig.cap="Sequence logos for the 5'-end 6-base motif, comparing MUT and NON-TARGET MUT fragments."}
plot_qqseqlogo_meme(
  df_fragments = results_df_basic,
  motif_type = "Start",
  motif_size = 6,
...
```{r plot_logo_single_group, fig.cap="Sequence logo showing the 4-base motifs from both the 5' and 3' ends for MUT fragments."}
plot_qqseqlogo_meme(
  df_fragments = results_df_basic,
  motif_type = "Both",
  motif_size = 4,
...
```{r plot_freq_barplot, fig.cap="Overall nucleotide frequency at the 5' end of MUT vs. NON-TARGET MUT fragments."}
plot_freq_barplot(
  df_fragments = results_df_basic,
  motif_size = 5,
  motif_type = "Start",
