...
process_fragment <- function(df_sam,
  ...
  # 1) Separate read1/read2
  reads       <- identify_reads(df_fragment_reads)
  read1_stats <- get_read_stats(reads$read1)
  read2_stats <- get_read_stats(reads$read2)
  
  # 2) Retrieve mutation information
  r_info1 <- get_mutation_info(mutation_type, pos, ref, alt, del_info, read1_stats)
  r_info2 <- get_mutation_info(mutation_type, pos, ref, alt, del_info, read2_stats)
  
  # 3) Compute distances and fragment size
