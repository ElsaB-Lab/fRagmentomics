...
plot_size_distribution <- function(
    ...
    .clean_args <- function(defaults, user) {
        ...
        for (nm in names(defaults)) {
            if (!nm %in% names(user)) user[[nm]] <- defaults[[nm]]
        }
        user
    }
    ...
    df_filtered <- dplyr::filter(
        df_fragments,
        !is.na(.data[[size_col]]) & is.finite(.data[[size_col]]) & !is.na(.data[[col_z]])
    )
    ...
    if (nrow(df_filtered) == 0) stop("No data remains after density QC. Check your filters.")
    ...
    if (show_histogram) {
        histo_defaults <- list(
            mapping  = ggplot2::aes(y = ggplot2::after_stat(density), fill = .data[[col_z]]),
            position = "identity",
