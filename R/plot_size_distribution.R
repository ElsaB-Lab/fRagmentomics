...
plot_size_distribution <- function(
    df_fragments, size_col = "Fragment_Size", col_z = "Fragment_Status_Simple",
    vals_z = NULL, histo_args = list(), density_args = list(), colors_z = NULL, show_histogram = FALSE,
    show_density = TRUE, x_limits = c(0, 600), histogram_binwidth = 5, show_nuc_peaks = TRUE,
    title = NULL, output_path = NA_character_, ggsave_params = list(
        width = 10, height = 7,
    ...
    # ---- base plot ----
    final_plot <- ggplot2::ggplot(df_filtered, ggplot2::aes(
        x = .data[[size_col]],
        color = .data[[col_z]]
    ))
    ...
    # histogram layer
    if (show_histogram) {
        histo_defaults <- list(mapping = ggplot2::aes(
            y = ggplot2::after_stat(density),
            fill = .data[[col_z]]
        ), position = "identity", binwidth = histogram_binwidth)
    ...
    if (show_nuc_peaks) {
        ...
        final_plot <- final_plot + ggplot2::geom_vline(
            xintercept = nuc_peaks, linetype = "dashed",
            color = "grey30"
        )
    }
    ...
    final_plot <- final_plot + ggplot2::labs(title = if (is.null(title) || is.na(title) ||
        ...
        title
    }, x = "Fragment Size (bp)", y = y_lab) + ggplot2::theme_bw(base_size = 14) +
        ggplot2::theme(
        ...
        ggplot2::scale_y_continuous(
            expand = ggplot2::expansion(mult = c(0, 0.1)) # 0% bottom gap, +10% headroom
        )
    ...
    if (!is.null(output_path) && is.character(output_path) && length(output_path) ==
        ...
        ggplot2::ggsave(
            filename = output_path, plot = final_plot, width = gp$width,
