...
#' 1.  **Coordinate Adjustment**: Converts the input 'pos' from 0-based to
#'      1-based if 'one_based' is 'FALSE'.
#' 2.  **Chromosome Naming**: Standardizes the chromosome name (e.g., '1' vs
#'      'chr1') to match the provided FASTA reference using
#'      'harmonize_chr_to_fasta'.
#' 3.  **Indel Padding**: For insertions and deletions, it prepends an "anchor"
#'      base from the reference genome to both 'ref' and 'alt' alleles to create
#'      a valid VCF-style representation. The 'pos' is adjusted accordingly for
#'      deletions. SNVs and MNVs are not modified.
#' 4.  **Validation**: After normalization, it confirms that the final 'ref'
#'      allele matches the sequence in the FASTA file at the new coordinates.
#'
...
normalize_to_vcf_rep <- function(
    ...
    if (nchar(ref_norm) != nchar(alt_norm)) {
        if (nchar(ref_norm) > nchar(alt_norm)) {
            # Deletion: In this case, we add the nucleotide before the pos of
            # interest.  Indeed, position meant to match the first position of
            # the del in the ref We also have to return the position before
            if (nchar(alt_norm) == 0) {
                anchor_base <- get_seq_from_fasta(
                    chr          = chr_norm,
                    # shift one base to the left to find the anchor
                    start        = pos_norm - 1,
                    # shift one base to the left to find the anchor
                    end          = pos_norm - 1,
            ...
            # Insertion. In this case, we add the nucleotide at the pos of
            # interest.  Indeed, position meant to match the position before the
            # ins in the ref
            if (nchar(ref_norm) == 0) {
                anchor_base <- get_seq_from_fasta(
                    chr          = chr_norm,
    ...
    if (ref_matches_fasta) {
        ...
        warntext <- sprintf(paste("Mismatch found between ref and fasta",
            "for (%s %d %s). Skipping the mutation."), chr, pos, ref)
        warning(warntext)
        return(NULL)
    }
}
...
#'
#' @description A helper function that removes common representations for
#' missing or null alleles (e.g., '-', '.', '_', 'NA') from reference and
#' alternate allele strings, converting them to empty strings ("").
#'
