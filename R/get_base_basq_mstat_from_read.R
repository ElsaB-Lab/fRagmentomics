...
#'
#' @keywords internal
get_base_basq_mstat_from_read <- function(
  ...
  if (read_index_at_pos %in% c(-1, -0.5)) {
    ...
    if (read_index_at_pos == -2) {
      ...
      basq <- substr(
        read_stats$QUAL, read_index_at_pos,
        read_index_at_pos
      )
    }
    ...
    while (nchar(base) < nchar(alt)) {
      shift <- shift + 1
      info_at_pos <- get_base_basq_from_read_at_pos(
        pos + shift,
...
get_base_basq_from_read_at_pos <- function(pos_cur, pos_pre, read_stats) {
  ...
  if (read_index_at_pos_cur == -1) {
    ...
    base <- substr(
      read_stats$SEQ, read_index_at_pos_pre + 1,
      nchar(read_stats$SEQ)
    )
    basq <- substr(
      read_stats$QUAL, read_index_at_pos_pre + 1,
