...
plot_ggseqlogo_meme <- function(
  ...
  names(motifs) <- paste0(
    names(motifs), " (N=", purrr::map_int(motifs, length),
    ")"
  )
  ...
  if (!is.null(colors_z)) {
    ...
    if (is.character(colors_z) && length(colors_z) == 1 && colors_z %in% rownames(RColorBrewer::brewer.pal.info)) {
      ...
      if (length(colors_z) != 4L) {
        stoptxt <- sprintf(paste(
          "An unnamed vector must contain",
          "exactly 4 colors (received: %d)."
        ), length(colors_z))
        stop(stoptxt)
      }
      ...
      stop(
        "'colors_z' must be NULL, a valid RColorBrewer palette name, ",
  ...
  # ---- CALL ggseqlogo ----
  args <- c(
    list(motifs), list(method = "prob"), if (is.null(dots$stack_width)) list(stack_width = 0.95) else list(),
  ...
  final_plot <- final_plot + ggplot2::geom_text(data = annotation_df, ggplot2::aes(
    x = x_pos,
    y = -0.1, label = label
  ), size = 5, inherit.aes = FALSE) + ggplot2::theme(
    ...
    axis.title.y = ggplot2::element_text(size = 14), strip.background = ggplot2::element_rect(
      fill = "white",
      color = NA
    ), panel.background = ggplot2::element_blank(), panel.grid = ggplot2::element_blank(),
    axis.line.x = ggplot2::element_blank(), axis.text.x = ggplot2::element_blank(),
    axis.ticks.x = ggplot2::element_blank(), plot.margin = ggplot2::margin(
      t = 5.5,
      r = 5.5, b = 20, l = 5.5, "pt"
    )
  )
