...
#'
#' @param mstat_1 The mutation status of read 1 (e.g., "MUT:C>T_high_conf", "WT", "AMB_low_coverage").
#'                Can be `NA` for no coverage.
#' @param mstat_2 The mutation status of read 2 (e.g., "MUT:C>T_high_conf", "WT", "AMB_low_coverage").
#'                Can be `NA` for no coverage.
#'
...
get_fragment_mutation_statuses <- function(mstat_1, mstat_2) {
  ...
  base_mstat_1 <- clean_status(mstat_1)
  base_mstat_2 <- clean_status(mstat_2)
  ...
  is_na1 <- is.na(mstat_1)
  is_na2 <- is.na(mstat_2)

  is_mut1 <- base_mstat_1 == "MUT"
  is_mut2 <- base_mstat_2 == "MUT"
  is_wt1 <- base_mstat_1 == "WT"
  is_wt2 <- base_mstat_2 == "WT"
  is_amb1 <- base_mstat_1 == "AMB"
  is_amb2 <- base_mstat_2 == "AMB"
  is_other_mut1 <- base_mstat_1 == "OTH"
  is_other_mut2 <- base_mstat_2 == "OTH"
  ...
  # 2. NA / MUT (and symmetrical)
    ...
    fragment_status_detail <- ifelse(is_na1, mstat_2, mstat_1)
  ...
  # 6. MUT / MUT
    ...
    fragment_status_detail <- combine_original_statuses(mstat_1, mstat_2)
    fragment_status_simple <- "MUT"
  }
