...
#'
#' @description This function serves as the main variant normalization pipeline. It iterates through a data frame of variants,
...
#'
#' @keywords internal
normalize_mut <- function(df_mut, fasta, fasta_fafile, one_based, apply_bcftools_norm, tmp_folder, verbose) {
  df_mut_norm <- data.frame(chr=character(), pos=integer(), ref=character(), alt=character())
  ...
  for (i in seq_len(nrow(df_mut))) {
    ...
    if (apply_bcftools_norm){
      df_mut_bcftools_norm <- apply_bcftools_norm(
        chr        = df_mut_vcf_norm$chr,
        pos        = df_mut_vcf_norm$pos,
        ref        = df_mut_vcf_norm$ref,
        alt        = df_mut_vcf_norm$alt,
        fasta      = fasta,
      ...
      for (i in seq_along(rename_map)) {
        ...
        if (old_name %in% colnames(df_mut_bcftools_norm)) {
          colnames(df_mut_bcftools_norm)[colnames(df_mut_bcftools_norm) == old_name] <- new_name
        }
      }
