...
plot_freq_barplot <- function(
  ...
  count_df <- dat %>%
    dplyr::mutate(a_count = stringr::str_count(motif, "A"), c_count = stringr::str_count(
      motif,
      "C"
    ), g_count = stringr::str_count(motif, "G"), t_count = stringr::str_count(
      motif,
      "T"
    ), Other = if (!drop_non_acgt) {
  ...
  # ---- Plot ----
  final_plot <- ggplot2::ggplot(plot_data, ggplot2::aes(
    x = group, y = frequency,
    fill = group
  )) +
    geom_col_layer +
    geom_errorbar_layer +
    ggplot2::facet_wrap(~nucleotide,
      scales = "free_x", nrow = 1
    ) +
    ggplot2::scale_x_discrete(labels = x_labels) +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = c(
      0,
      y_max
    ), expand = c(0, 0)) +
    ...
    ggplot2::labs(
      title = title_txt,
      x = NULL, y = "Overall Frequency", caption = plot_caption
    ) +
    ggplot2::theme(
      legend.position = "right",
      axis.text.x = ggplot2::element_blank(), axis.ticks.x = ggplot2::element_blank(),
      plot.caption = ggplot2::element_text(hjust = 0, size = 10, face = "italic"),
      strip.text = ggplot2::element_text(face = "bold", size = 12), strip.background = ggplot2::element_rect(
        fill = "white",
        color = NA
