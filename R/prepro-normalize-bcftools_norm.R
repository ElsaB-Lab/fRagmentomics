...
apply_bcftools_norm <- function(chr, pos, ref, alt, fasta) {
    ...
    if (ref == alt) {
      normalized_variants <- data.frame(chr, pos, ref, alt, stringsAsFactors = FALSE)
    } else {  
      ...
      cmd <- sprintf("bcftools norm -m +both -d exact --check REF,ATL -f %s -o %s %s",
                      shQuote(fasta),
                      shQuote(tmp_out_vcf),
                      shQuote(tmp_vcf))
      ...
      exit_status <- tryCatch({
          ...
          warning(sprintf("Error running bcftools norm for %s:%d:%s:%s - %s", chr, pos, ref, alt, e$message))
          return(1)  # Fail
      })
      ...
      if (exit_status != 0) {
            ...
            if (!file.exists(tmp_out_vcf) || file.info(tmp_out_vcf)$size == 0) {
                warning(sprintf("bcftools norm produced an empty VCF for %s:%d:%s:%s", chr, pos, ref, alt))
                return(NULL)
            }
